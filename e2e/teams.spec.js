...
test.describe('Team Management — List Page', () => {
  ...
  test('team cards display team names', async ({ page }) => {
    await expect(page.locator('.team-card-title').filter({ hasText: 'Engineering Team' })).toBeVisible();
...
test.describe('Team Management — Team Detail', () => {
  ...
  test('team detail page renders correctly', async ({ page }) => {
    ...
    await expect(page.locator('.page-subtitle').filter({ hasText: 'Engineering Team' })).toBeVisible();
  });
  ...
  test('clicking Edit navigates to edit page', async ({ page }) => {
    ...
    const editBtn = page.locator('.header-right').locator('button, a').filter({ hasText: /Edit|pencil/ }).first();
    await editBtn.click();
