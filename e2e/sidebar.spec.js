...
test.describe('Sidebar Navigation', () => {
  ...
  test('sidebar is visible with all nav items', async ({ page }) => {
    ...
    for (const label of navLabels) {
      await expect(page.locator('.sidebar-item-label').filter({ hasText: label }).first()).toBeVisible();
    }
  });
  ...
  test('active item changes on navigation', async ({ page }) => {
    ...
    const coursesItem = page.locator('.sidebar-item').filter({
    ...
    const dashboardItem = page.locator('.sidebar-item').filter({
