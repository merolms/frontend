...
const LearningPathForm = ({ learningPathId }: LearningPathFormProps) => {
  const router = useRouter();
  const { addToast } = useToast();
  const isEdit = !!learningPathId;
  ...
  const loadPath = async () => {
    try {
      ...
      // const data = await fetchLearningPathById(learningPathId);
      // if (data) {
  ...
  const applyTemplate = (template: any) => {
    setForm((p) => ({
      ...
      courses: template.courses.map((c: any, i: number) => ({ ...c, order: i + 1 })),
      versionNotes: "",
      isPublic: false,
    }));
  ...
  const bulkMoveCourses = (direction: "up" | "down") => {
    if (selectedCourses.size === 0) return;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEdit) {
        // await updateLearningPath(learningPathId, form);
    ...
    >
      <form onSubmit={handleSubmit} className="max-w-4xl">
        ...
        <div className="flex justify-end gap-3">
          <Button type="button" variant="default" onClick={() => router.push("/learning-paths")}>
            Cancel
          </Button>
