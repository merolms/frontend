...
const AssignmentDetail = ({ assignmentId }: AssignmentDetailProps) => {
  ...
  const handlePublish = async () => {
    try {
      const { publishAssignment } = await import("@/services/assignmentService");
                ...
                <div className="space-y-2">
                  ...
                  {canGrade && (
                    <Button
                      variant="outline"
                      className="w-full"
                      onClick={() => router.push(`/assignments/${assignmentId}/grade`)}
                    >
