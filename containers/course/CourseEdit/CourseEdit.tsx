...
const CourseEdit = ({ courseId }: CourseEditProps) => {
  ...
  const { data: course, isLoading: fetching, error: courseError } = useCourse(courseId);
  ...
  return (
    <DashboardLayout title="Edit Course" subtitle="Update the course metadata and settings">
      <div className="text-text-muted mb-4 flex items-center gap-1 text-xs">
        <button onClick={() => router.push("/courses")} className="text-primary hover:underline">
          Courses
        </button>
        <span>/</span>
        <button
