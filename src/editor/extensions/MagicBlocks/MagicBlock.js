...
export default Node.create({
  ...
  addAttributes() {
    return {
      ...
      height: {
        default: 400,
        parseHTML: (element) => { const h = element.getAttribute('data-height'); return h ? parseInt(h, 10) : 400 },
        renderHTML: (attributes) => ({ 'data-height': attributes.height }),
      },
    }
  },
  parseHTML() { return [{ tag: 'block-magic' }] },
  renderHTML({ HTMLAttributes }) { return ['block-magic', mergeAttributes(HTMLAttributes)] },
  addNodeView() { return ReactNodeViewRenderer(MagicBlockComponent) },
})
