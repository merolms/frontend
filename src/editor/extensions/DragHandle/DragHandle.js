...
function createDragHandlePlugin() {
  ...
  function init(view) {
    ...
    function handleMouseMove(event) {
      ...
      if (block) { hoveredBlock = block; positionHandle(block); showHandle() }
    }
    function handleMouseLeave() { scheduleHide() }
    function handleHandleEnter() { clearHideTimeout() }
    function handleHandleLeave() { if (draggedBlockPos === null) scheduleHide() }
    ...
    function handleActionClick(event) {
      ...
      if (action === 'delete') {
        ...
        const emptyNode = view.state.schema.nodes[node.type.name].create(node.attrs, null, node.marks)
    ...
    function handleDragStart(event) {
      ...
      try {
        ...
        dragImage.style.cssText = 'position:absolute;top:-9999px;opacity:0.8;background:white;padding:8px;border-radius:4px;max-width:300px;'
    ...
    function getDropTarget(clientY, view) {
      ...
      for (const block of children) {
        ...
        if (clientY >= rect.top - 30 && clientY <= rect.bottom + 30) {
          ...
          if (!closest || distance < closest.distance) closest = { element: block, distance, position }
        }
      }
      ...
      try {
        ...
        return { pos: closest.position === 'before' ? nodePos : nodePos + blockNode.nodeSize, rect: closest.element.getBoundingClientRect(), position: closest.position }
      } catch { return null }
    }
    ...
    function showHandle() { clearHideTimeout(); dragHandle?.classList.add('visible') }
    function hideHandle() { dragHandle?.classList.remove('visible'); hoveredBlock = null }
    function scheduleHide() { clearHideTimeout(); hideTimeout = setTimeout(hideHandle, 200) }
...
export const DragHandle = Extension.create({
  name: 'dragHandle',
  addProseMirrorPlugins() { return [createDragHandlePlugin()] },
})
