...
const PasteFileHandler = Extension.create<PasteFileHandlerOptions>({
  ...
  addProseMirrorPlugins() {
    ...
    const uploadAndReplace = async (file: File, blockType: string, nodePos: number): Promise<void> => {
      try {
        // Resolve lessonId from editor storage (set by the lessonId prop via a tiny extension)
        const lessonId: string | number | null | undefined =
          (editor.storage as Record<string, Record<string, unknown>>).pasteFileHandler?.lessonId;

        if (!lessonId) return;

        const { url } = (await uploadEditorMedia(file, lessonId, "temp_upload")) as UploadResult;

        const remoteAttr: Record<string, unknown> =
    ...
    const handleFiles = (files: FileList | File[], pos?: number): boolean => {
      let handled = false;
