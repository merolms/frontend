...
const CourseBuilder = () => {
        ...
        if (lessonId) {
          ...
          if (found) {
            ...
            if ((result.total || lessonList.length) === 0) {
              ...
              navigate(`/courses/${id}/builder`, { replace: true });
              return;
            }
  ...
  const handlePageChange = (newPage) => {
    if (newPage >= 1 && newPage <= pagination.totalPages) {
      ...
      const queryString = searchParams.toString();
      navigate(`/courses/${id}/builder${queryString ? `?${queryString}` : ""}`, { replace: true });
    }
  };
  ...
  const loadLesson = useCallback(
    async (lesson, currentPage = null) => {
      ...
      const queryString = searchParams.toString();
      navigate(`/courses/${id}/builder/${lesson.id}${queryString ? `?${queryString}` : ""}`, {
        replace: true,
      });
