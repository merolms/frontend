...
const toInlineContent = (content) => {
  ...
  if (Array.isArray(content)) {
    return content.filter((c) => c && c.type).map((c) =>
...
const LessonPanel = ({ lessons = [], selectedLessonId, onSelectLesson, width = 280 }) => {
  return (
    <aside className="flex flex-col overflow-hidden" style={{ width, flexShrink: 0, background: t('bg-sidebar'), borderRight: `1px solid ${t('border-primary')}` }}>
      <div className="flex items-center justify-between px-4 py-3 flex-shrink-0" style={{ background: t('bg-secondary'), borderBottom: `1px solid ${t('border-primary')}` }}>
        <div className="flex items-center gap-2">
          <BookOpen size={14} style={{ color: t('text-muted') }} />
          <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', color: t('text-muted') }}>Lessons</span>
        </div>
      </div>
      <div className="flex-1 overflow-y-auto" style={{ padding: '8px' }}>
        {lessons.length === 0 && (
          <div style={{ textAlign: 'center', padding: '32px 12px', color: t('text-disabled'), fontSize: 12 }}>No lessons yet</div>
        )}
          ...
          return (
            <div
              ...
              style={{
                ...
                borderRadius: t('radius-md'), cursor: 'pointer', marginBottom: 2,
            ...
            >
              ...
              }}>{lesson.title || `Lesson ${i + 1}`}</span>
            </div>
          );
        })}
      </div>
      {lessons.length > 0 && (
        <div className="flex-shrink-0 px-4 py-2" style={{ borderTop: `1px solid ${t('border-primary')}`, background: t('bg-secondary') }}>
          <span style={{ fontSize: 10, color: t('text-disabled') }}>{lessons.length} {lessons.length === 1 ? 'lesson' : 'lessons'}</span>
        </div>
      )}
    </aside>
  );
...
const CoursePreview = () => {
    ...
    try {
      ...
      if (autosave?.snapshot) {
        ...
        return Array.isArray(snap) ? snap
          : snap.content ? (typeof snap.content === 'string' ? JSON.parse(snap.content) : snap.content)
          : [snap];
      }
    } catch { /* ignore */ }
    try {
      const blocks = await fetchLessonBlocks(lesson.id);
      if (blocks.length > 0) return blocks.map((b) => ({
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setLessons(sorted);
        if (sorted.length > 0) {
          const target = lessonId ? sorted.find((x) => String(x.id) === String(lessonId)) || sorted[0] : sorted[0];
  ...
  const handleSelectLesson = useCallback(async (lId) => {
  ...
  const goToPrev = () => { if (canGoPrev) handleSelectLesson(lessons[lessonIndex - 1].id); };
  const goToNext = () => { if (canGoNext) handleSelectLesson(lessons[lessonIndex + 1].id); };
