...
const TeamDetail = () => {
  ...
  const loadData = async () => {
    try {
      ...
      setTeam(tData); setMembers(mData || []);
  ...
  if (error || !team) {
    return (
      <DashboardLayout>
        <div className="flex items-center gap-2 text-error py-4"><AlertCircle size={14} /> {error || 'Team not found'}</div>
    ...
    <>
      <DashboardLayout title={team.name} subtitle={team.description}>
        ...
        <div className="flex items-center justify-end gap-2 mb-4">
          <Button size="sm" onClick={() => setShowMemberModal(true)}><Plus size={14} /> Add Member</Button>
          <Button variant="default" size="sm" onClick={() => navigate(`/teams/${id}/edit`)}><Pencil size={14} /> Edit</Button>
          <Button variant="default" size="sm" onClick={() => setDeleteTarget(team)}><Trash2 size={14} /> Delete</Button>
        </div>
        ...
        <div className="grid grid-cols-12 gap-4">
          <div className="col-span-8">
            <Paper className="p-6">
              <h3 className="text-sm font-semibold text-text-primary mb-3">Team Members ({members.length})</h3>
              {members.length === 0 ? (
                ...
                <div className="space-y-2">
                  {members.map((m) => {
                    ...
                    return (
                      <div key={userId} className="flex items-center justify-between rounded-lg p-3" style={{ background: t('bg-secondary') }}>
                        <div className="flex items-center gap-3">
                          <Avatar className="h-8 w-8">
                            <AvatarImage src={m.avatar} />
                            <AvatarFallback>{(m.userName?.[0] || 'U').toUpperCase()}</AvatarFallback>
                          </Avatar>
                          <div>
                            <div className="text-xs font-semibold text-text-primary">{m.userName || 'Unknown'}</div>
                            <div className="text-[11px] text-text-muted">{m.userEmail}</div>
                          </div>
                        </div>
                        <Badge>{m.role || '—'}</Badge>
                      </div>
          ...
          <div className="col-span-4">
            <Paper className="p-6">
              ...
              <div className="space-y-3">
                <div>
                  <div className="text-xs font-semibold text-text-primary">Status</div>
                  <Badge variant={team.status === 1 ? 'green' : 'gray'}>{team.status === 1 ? 'Active' : 'Inactive'}</Badge>
                </div>
                <div>
                  <div className="text-xs font-semibold text-text-primary">Members</div>
                  <div className="text-xs text-text-muted">{members.length}</div>
                </div>
              </div>
            </Paper>
          </div>
