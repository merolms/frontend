...
const Badge = React.forwardRef(({ className, variant = 'default', children, onClose, ...props }, ref) => {
  return (
    <span
      ref={ref}
      className={cn(
    ...
    >
      {children}
      {onClose && (
        <button onClick={onClose} className="ml-0.5 hover:opacity-70 cursor-pointer">
          <X size={10} />
        </button>
      )}
    </span>
